...
export class WingedEdgeGraph {
    ...
    static getMidpointAndSplitLine(polyline:Array<Point>):SplitLineObj {
    ...
    static findPerpendicularPoints(p1:Point, p2:Point, d:number):PerpPointsObj {
        // Step 1: Compute the direction vector (from (x1, y1) to (x2, y2))
        let dirX = p2.x - p1.x;
        let dirY = p2.y - p1.y;
        
        // Step 2: Calculate the scale factor to ensure the points are distance `d` away
        let scale = d / Math.sqrt(dirX * dirX + dirY * dirY);
    
        // Step 3: Calculate the perpendicular directions
        // Counterclockwise direction (rotate 90 degrees)
        let leftX = p1.x - dirY * scale;
        let leftY = p1.y + dirX * scale;
        
        // Clockwise direction (rotate -90 degrees)
        let rightX = p1.x + dirY * scale;
        let rightY = p1.y - dirX * scale;
    ...
    static getSuccessors(node:WdsNode):SuccessorsObj|undefined {
        ...
        if (WingedEdgeGraph.getDeg(nodeIncidentToWinged) === 3) {
            ...
            const wingedEdges = nodeIncidentToWinged.outEdges.filter(edge => edge instanceof WdsWingedEdge)
            if (wingedEdges.length === 2) {
                if (wingedEdges[0] instanceof WdsWingedEdge && wingedEdges[0].rightPred === outEdge) {
                    return {leftSucc: wingedEdges[1], rightSucc: wingedEdges[0]}
                } else {
                    return {leftSucc: wingedEdges[0], rightSucc: wingedEdges[1]}
                }
            }
        }
        console.log("getSucessors Unexpected Error")
        return undefined
    }

    // get reversed edge O(1)
    ...
    // nodes are distinct from edges in terms of points
    // O(n), where n = # of nodes in graph
    addSuperEdge(n1:WdsNode, n2:WdsNode, polyline:Array<Point>):TriPointTuple|undefined {
        const {midpoint, firstLine, secondLine} = WingedEdgeGraph.getMidpointAndSplitLine(polyline)
        ...
        internalOfN2.outEdges.filter(edge => edge.dst !== n2)
        internalOfN1.outEdges.filter(edge => edge.dst !== n1)
...
const nodes = WingedEdgeGraph.createCross(new Point(600, 400)).nodes
// nodes.push(...WingedEdgeGraph.createCross(new Point(900, 400)).nodes)

const wds = new WingedEdgeGraph(nodes)
wds.addSuperEdge(wds.nodes[0], wds.nodes[1], [new Point(620,-360), new Point(640,-360), new Point(640,-380)])
wds.addSuperEdge(wds.nodes[1], wds.nodes[2], [new Point(560,-360), new Point(560,-340), new Point(660,-340)])
for (let node of wds.nodes) {
    console.log(wds.getFace(node))
}
